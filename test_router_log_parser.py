...
def log_parser(request):
    log_parser = RouterLogParser("h00", {}, db_name='test')
...
class TestUpdateDevList():
    ...
    def test_new_devs(self, log_parser):
        ...
        device = log_parser.dev_list.find_one({"dev_name": "Taos-iPhone-5S"})
        count = log_parser.dev_list.find({'last_updated': my_date}).count()
        dev_list_latest_updated = log_parser.prop.find_one({'key': 'dev_list_latest_updated'})['val']
    ...
    def test_update_devs(self, log_parser):
        ...
        with open(filename) as f:
            log_parser.update_dev_list(f, timestamp)
        tao_iphone = log_parser.dev_list.find_one({"dev_name": "Taos-iPhone-5S"}, sort=[('last_updated', DESCENDING)])
        idea_pc = log_parser.dev_list.find_one({"dev_name": "idea-PC"})
        latest_count = log_parser.dev_list.find({'last_updated': my_date}).count()
...
class TestFindDevName():
    ...
    def access_record(self):
        return {"accessed_on": datetime.utcfromtimestamp(1397437700),
    ...
    def init_dev_list(self, test_dev_name, log_parser):
        ...
        my_devs = [{"dev_name": test_dev_name,
                    "ip": "192.168.1.20",
    ...
    def test_single_dev(self, log_parser, access_record, test_dev_name, init_dev_list):
        ...
        """
        new_dev = {"dev_name": test_dev_name,
                   "ip": "192.168.1.20",
                   "last_updated": datetime.utcfromtimestamp(1397433316)}
...
class TestWriteAccessLog():
    ...
    def test_process_delayed_access_records(self, log_parser, reset_collections):
        ...
        r1 = log_parser.access_log.find_one({"accessed_on": datetime.utcfromtimestamp(1397437705)})
        r2 = log_parser.access_log.find_one({"accessed_on": datetime.utcfromtimestamp(1397437710)})
        r3 = log_parser.access_log.find_one({"accessed_on": datetime.utcfromtimestamp(1397437805)})
